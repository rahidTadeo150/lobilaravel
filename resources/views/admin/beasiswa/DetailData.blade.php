...
@section('Content')
    <div class=" h-96 overflow-hidden relative -ml-4 -mr-4">
        <img class=" h-80 absolute shadow-xl border border-gray-700 z-20 rounded-md -translate-y-[50%] -translate-x-[50%] top-[50%] left-[50%]" src="\storage{{ $Data->foto_beasiswa }}">
        <div class="w-full h-full bg-black opacity-40 absolute z-10"></div>
        <div class="w-full overflow-hidden">
            <img class="bg-contain z-0 w-full -translate-y-1/4 blur-sm" src="\storage{{ $Data->foto_beasiswa }}">
        </div>
    </div>
    <div class="w-full flex flex-row justify-between mt-8">
        <div class="w-[350px] bg-white shadow-lg border border-gray-300 border-l-4 border-l-[#315BC9] pt-8 pb-14 px-6 rounded-md">
            <div>
                <p class="text-3xl font-bold mb-2">{{ $Data->nama_beasiswa }}</p>
        ...
        <div class="w-[650px] bg-white shadow-lg border border-gray-300 border-l-4 border-l-[#315BC9] rounded-md p-6">
            ...
            <div class="w-full flex flex-row mt-10">
                <a href="/edit-data-beasiswa?IdObject={{ $Data->id }}&IdInstansi={{ $Data->Instansi->id }}">
                ...
                <form action="/delete-data-beasiswa" method="post">
                    @csrf
                    @method('delete')
                    <input type="hidden" name="IdBeasiswa" value="{{ $Data->id }}">
