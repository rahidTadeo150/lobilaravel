...
@section('Content')
    <div>
        ...
        <p class="text-sm text-gray-700 font-normal">Form Edit Data Beasiswa Lobi Poliwangi</p>
                ...
                <div class="flex flex-row justify-between items-center px-5">
                    <div>
                        ...
                        <div class="relative z-0 w-64 mb-9">
                            <input name="TanggalPendaftaran" value="{{ $DataBeasiswa->tanggal_pendaftaran }}" autocomplete="off" type="date" id="floating_standard" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" />
                        ...
                        <div class="relative z-0 w-64 mb-9">
                            <input name="TanggalPenutupan" value="{{ $DataBeasiswa->tanggal_penutupan }}" autocomplete="off" type="date" id="floating_standard" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" />
                        ...
                        <div class="relative z-0 w-80 mb-9">
                            <select name="Tingkatan" autocomplete="off" type="date" id="floating_standard" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer">
                                ...
                                <option {{ $DataBeasiswa->Tingkatan->id == $Tingkatan->id ? 'selected' : '' }} value="{{ $Tingkatan->id }}" class="text-sm font-normal pl-2 pr-2">{{ $Tingkatan->tingkatan }}</option>
                                @endforeach
                            </select>
                            @error ('Tingkatan')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oops!</span>{{ $message }}</p>
                    ...
                    <div class="flex flex-col items-center gap-y-3">
                        ...
                        <div class="flex flex-col overflow-hidden justify-center items-center relative w-[330px] h-[330px] border-2 border-dashed rounded-md border-black">
                            ...
                            <img class="absolute z-10 bg-contain h-full w-full" src="\storage\{{ $DataBeasiswa->foto_beasiswa }}">
                        </div>
                ...
                <div class="flex flex-row">
                    ...
                    <button id="SubmitButton" onclick="confirm('Ingin Menyimpan Instansi Ini')" type="submit" class="disabled-submit-button ml-5 p-3 pl-8 pr-8 mt-10 mb-10 w-96 rounded-md" disabled>
                        <div class="flex flex-row justify-center items-center w-full h-full gap-x-2">
                            ...
                            <p class="text-gray-100 font-semibold">Simpan Pembaruan Beasiswa</p>
                        </div>
                    </button>
                </div>
            </form>
