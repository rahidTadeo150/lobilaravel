...
@section('Content')
    <div>
        ...
        <div>
            <form action="/updating-data-lomba" method="post" enctype="multipart/form-data">
                @csrf
                @method('put')
                ...
                <div class="flex flex-row justify-between items-center px-5">
                    <div>
                        ...
                        <div class="relative z-0 w-96 mb-9">
                            <input name="LinkPendaftaran" value="{{ $DataLomba->link_pendaftaran }}" autocomplete="off" type="text" id="floating_standard" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" />
                            <label for="floating_standard" class="absolute text-sm text-gray-900 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto">Link Pendaftaran Lomba</label>
                            @error ('LinkPendaftaran')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oops!</span>{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="relative z-0 w-[280px] mb-9">
                            <input name="TanggalPendaftaran" value="{{ $DataLomba->tanggal_pendaftaran }}" autocomplete="off" type="date" id="floating_standard" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" />
                        ...
                        <div class="relative z-0 w-[280px] mb-9">
                            <input name="TanggalPenutupan" value="{{ $DataLomba->tanggal_penutupan }}" autocomplete="off" type="date" id="floating_standard" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" />
                        ...
                        <div>
                            <label for="message" class="block mb-2 text-sm font-medium text-gray-900">Persyaratan Lomba</label>
                            <textarea name="Persyaratan" rows="4" class="block p-2.5 w-full outline-0 text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Write your thoughts here...">{{ $DataLomba->persyaratan }}</textarea>
                            @error ('Persyaratan')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oops!</span>{{ $message }}</p>
                            @enderror
                        </div>
